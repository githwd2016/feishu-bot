...
相同 `PR + head SHA + 当前账号` 只执行一次；PR 推送新提交后允许重新审查。失败会在后续扫描重试，连续三次失败后停止该版本重试并在群中告警。
...
同一 head SHA 的重复 initial 请求只执行一次；显式复审、协议中的新 cycle 或新的 head SHA 会开启新一轮。重复请求在原任务运行时会收到“已在处理中”，原任务结束后则重放其结果。
