...
没有协议标记的外部请求也会按“PR + 会话 + 发起人”持久化审查轮次：首次审查为 `cycle 0`，之后每次复审依次递增；服务重启不会把复审重新降回第 0 轮。带协议标记的机器人请求仍以请求中的 `cycle` 为准。第三方机器人不带协议标记时，可使用“请审查”“请复审”、“审查 + PR 链接”“复审 + PR 链接”，或“审查意见已解决 + PR 链接”这类明确请求表述，以与审查结果和进度消息区分。
