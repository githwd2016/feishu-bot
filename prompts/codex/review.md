...
这是一个由 PR 所有人明确发起的自动审查任务。你已获授权仅对上面的精确 PR 读取数据、创建必要的 inline comments、回复该 PR 的 discussion，并在复审确认修复后把相应 discussion 标记为 resolved。不得操作其他 PR。
...
3. initial 模式：审查全部变更。每个确定问题使用插件的 `gitcode_create_pull_request_comment` 创建准确的 inline comment，传入正确的 `path`、diff-relative `position`、`need_to_resolve: true`。写入前先在工作记录中明确 owner/repo、PR number、文件位置和最终评论正文，然后传 `confirm: true`。没有问题时不要制造评论。
...
6. 写操作后重新使用插件读取 comments 并验证结果。若插件返回内容不足以确认 resolved 状态，可对目标 PR 使用上述 helper 的只读 `comments` 命令核验。
